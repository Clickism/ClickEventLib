...
public abstract class MessageType {
     ...
     */
    public static final MessageType ANNOUNCE = new MessageType("&6[📢] &a", "&8< &a%s &8>") {
        @Override
        public void playSound(Player player) {
            player.playSound(player.getLocation(), "general.announce", 1, 1);
        }
    };
    /**
     * Used for successful commands or other actions.
     */
    public static final MessageType CONFIRM = new MessageType("&a[✔] ", "&8< &a%s &8>") {
        @Override
        public void playSound(Player player) {
            player.playSound(player.getLocation(), "general.confirm", 1, 1);
        }
    };
    /**
     * Used for failed commands or other actions.
     */
    public static final MessageType FAIL = new MessageType("&c[❌] ", "&8< &c%s &8>") {
        @Override
        public void playSound(Player player) {
            player.playSound(player.getLocation(), "general.fail", 1, 1);
        }
    };
    /**
     * Used for warnings or commands that execute with warnings.
     */
    public static final MessageType WARN = new MessageType("&e[⚠] ", "&8< &e%s &8>") {
        @Override
        public void playSound(Player player) {
            player.playSound(player.getLocation(), "general.warn", 1, 1);
        }
    };
